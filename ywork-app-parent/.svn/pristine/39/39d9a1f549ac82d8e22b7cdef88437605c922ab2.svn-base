...
import me.ywork.org.util.Ding2OrgUtil;
...
public class Ding2OrgSyncJob implements InitializingBean, Runnable{
	 ...
	 * 线程数
	 */
	private final static int cores = 8;//4 * Runtime.getRuntime().availableProcessors();
	...
	public void run() {
		...
		while (!this.isShuttingDown) {
			try {
				...
				logger.info("本轮已结束，正在等待下一轮同步组织架构....");
		...
		executor.execute(new Runnable() {
			...
			public void run() {
				try {
					...
					if(corp != null){
						logger.info("sync corp name:"+corp.getFdCorpName());
	...
	private Map<String, DingOrgDept> tranDept(DingSuiteThirdForSync third ,DingOrgCorp dingOrg) {
		...
		//更新层级ID
		for (DeptVo qyDept : deptList) {
			updateHierarchy(qyDept ,dingOrg , deptMap);
		}
	...
	private List<DeptVo> listDept(DingSuiteThirdForSync third ,DingOrgCorp corp) {
		...
		try {
			...
			if(deptJson==null){
				logger.warn("---------没有缓存数据,不同步公司：" + third.getCorpName());
				return null;
			}
	 ...
	 * @param deptMap
	 */
	private void updateHierarchy(DeptVo qyDept, DingOrgCorp dingOrg ,Map<String, DingOrgDept> deptMap) {
		DingOrgDept dept = deptMap.get(String.valueOf(qyDept.id));
	...
	private void tranPerson(DingSuiteThirdForSync third ,DingOrgCorp corp ,JSONObject userDetailJson,Map<String, DingOrgDept> deptMaps) {
		try {
			...
			if(userDetailJson==null){
				logger.warn("---------没有缓存数据,不同步公司人员：" + third.getCorpName());
				return;
			}
			...
			for (int i = 0; i < userDetailArray.size(); i++) {
				...
				syncUserByJson(third.getSuiteId() , corp, userDetailInfoJson, deptMaps, dingOrgElementRepository , dingSuiteThirdMainRepositroy,null);
			}
		}catch(Exception e){
			e.printStackTrace();
		}
	...
	public static String syncUserByJson(String suiteId, DingOrgCorp corp ,
		...
		try{
			...
			//orgUser.setFdIsBusiness(active);
			...
			for (int j = 0; j < dept.length; j++) {
				...
				if(orgActor == null){
					...
					orgActor.setFdIsAvailable(true);
					...
					orgActor.setFdAlterTime(corp.getFdLastSyncTime());
					orgActor.setFdNo(jobnumber);
					dingOrgElementRepository.updateAlterTime(orgActor);
				}
	...
	private JSONObject getDingDeptList(DingSuiteThirdForSync third) {
		...
		//String key = DINGDEPTLIST + third.getCorpId();
...
//	}
	...
	private JSONObject getDetailListDeptUsers(DingSuiteThirdForSync third ,String deptId){
		JSONObject json=null;
	    //String key = GETDETAILLISTDEPTUSERS + third.getCorpId() + deptId ;
		//json = (JSONObject) cacheableService.getRawObjectFromCache(key);
		//if(json==null){
		long startTime = System.currentTimeMillis();
		json = dingApiService.getDetailListDeptUsers(third, deptId);
		if(json!=null){
			//cacheableService.setRawObjectInCache(key, json,1,TimeUnit.DAYS);//24小时
			if(logger.isTraceEnabled()){
				logger.trace(json.toJSONString());
				logger.trace("获取人员详细信息耗时:("+third.getCorpName()+") --" + ((System.currentTimeMillis() - startTime)/1000) + "秒");
			}
		}
		//}
		return json;
