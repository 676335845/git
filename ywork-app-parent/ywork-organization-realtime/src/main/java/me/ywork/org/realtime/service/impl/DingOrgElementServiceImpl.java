...
/*******************华丽分割线****************************/
	...
	public String syncElement(String suiteId, String corpId) {
		...
		if(corp != null){
			...
			//部门及人员
			Map<String, DingOrgDept> deptMaps = tranDept(suiteThirdMain, corp);
	...
	//部门同步
	private Map<String, DingOrgDept> tranDept(DingSuiteThirdMain third ,DingOrgCorp dingOrg) {
		// 获取钉钉部门列表
	...
	private void tranPerson(DingSuiteThirdMain third ,DingOrgCorp corp ,JSONObject userDetailJson,Map<String, DingOrgDept> deptMaps) {
		try {
			...
			for (int i = 0; i < userDetailArray.size(); i++) {
				...
				syncUserByJson(third.getSuiteId() , corp, userDetailInfoJson, deptMaps,null);
			}
		}catch(Exception e){
			e.printStackTrace();
		}
