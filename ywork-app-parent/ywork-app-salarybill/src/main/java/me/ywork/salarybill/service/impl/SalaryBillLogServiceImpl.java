...
public class SalaryBillLogServiceImpl implements SalaryBillLogService{
	...
	@Override
	public PageData<SalaryBillLogModel> getSalaryBillLog(String companyId, Pageable pageable) {
		// 当请求第一页时统计满足条件的总数量
		Long totalCount = 0L;
		if (pageable.getPageNo() <= 1) {
			totalCount = this.salaryLogRepository
					.getSalaryBillLogCount(companyId);
